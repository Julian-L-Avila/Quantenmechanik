...
def analyze_spring_data(mass_g, length_cm, g_accel=GRAVITY_ACCELERATION_G):
    ...
    Gibt k (N/m), L0 (m), Steigung, Y-Achsenabschnitt und R-Quadrat zurück.
    """
    ...
    return spring_constant_Nm, rest_length_m, slope, intercept, r_value**2
...
def plot_multiple_spring_data(spring_data_for_plot):
    ...
    colors = ['blue', 'green', 'orange', 'purple'] # Farben für verschiedene Federn
    ...
    for i, spring_info in enumerate(spring_data_for_plot):
        ...
        # Dazu müssen wir Steigung und Achsenabschnitt entsprechend umrechnen.
        # Steigung_cm_g = Steigung_m_kg * (100 cm / 1 m) / (1000 g / 1 kg) = Steigung_m_kg / 10
        # Achsenabschnitt_cm = Achsenabschnitt_m * 100 cm / 1 m
...
def save_multiple_results(all_results, output_file=OUTPUT_RESULTS_NAME):
    ...
    with open(output_file, 'w') as f:
        ...
        for i, res in enumerate(all_results):
            ...
            f.write(f"Federkonstante (k): {res['spring_constant']:.4f} N/m\n")
...
if __name__ == "__main__":
    ...
    for i, file_name in enumerate(TSV_FILE_NAMES):
        ...
        if data is not None:
            ...
            k_val, l0_val, reg_slope, reg_intercept, r_sq = analyze_spring_data(masses, lengths)
